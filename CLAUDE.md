...
- TIMING MISMATCH — must be fixed before charging: the server dispatches question 1 at `phase_start` + 3000ms (`setTimeout` in `startPhase`), but the client will not display it until `introEndsAt` = arrival + 3850ms (`runIntroCountdown`, index.html ~1332). The charge therefore fires ~850ms + network latency BEFORE the player sees question 1. Fix: raise the server delay for PHASE 0 ONLY to ~4500ms; phases 1 and 2 keep 3000ms.
